    ...
    public void handle(Player player, Packet packet) {
		...
		if (unpacked.startsWith("!price")) {
			...
			if (option.isPresent()) {
				...
				player.getActionSender().sendMessage("<col=ff0000>" + item.getDefinition2().getName() + " currently costs " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(item.getPrice()) + ".");
			}
		}
