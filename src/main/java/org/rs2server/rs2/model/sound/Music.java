...
public class Music {
	...
	public static void playMusic(Player player) {
		...
		for (int i = 0; i < songs.length; i++) {
			if (unlocked[songs[i].array] == false && which == songs[i]) {
				//player.sendMessage("<col=992D2D>You have unlocked a new music track: " + songs[i].name + ".");
	...
	public static boolean[] unlocked = new boolean[1000];

	// name, swX, swY, neX, neY, song id, childId, arraySlot
	public static final Music[] songs = {

			new Music("Pick & Shovel", 3710, 5630, 3770, 5690, 426, 100, 384), // Motherlode
																				// Mine
			...
			new Music("Barbarianism", 3072, 3392, 3135, 3455, 141, 4864,
					27), /** TODO: 4868 **/
			...
			new Music("Arcane", 1613, 3650, 1671, 3695, 509, -1, 385), // Zeah
																		// home
																		// place
	};
}