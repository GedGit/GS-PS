...
public class AhrimTheBlighted extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), victim.getCentreLocation(), 156, 45, 50,
				clientSpeed, 43, 35, victim.getProjectileLockonIndex(), 10, 48));
		attacker.getCombatState().setSpellDelay(5);
		attacker.getCombatState().setAttackDelay(4);
		int randomHit = Misc.random(
				damage(25, attacker, victim, AttackType.MAGIC, Skills.MAGIC, Prayers.PROTECT_FROM_MAGIC, false, false));
