...
public class Skotizo extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		style = attacker.getLocation().distance(victim.getLocation()) <= 2 ? CombatStyle.MELEE
				: Misc.random(9) <= 4 ? CombatStyle.MAGIC : CombatStyle.RANGE;
		...
		default:
			...
			World.getWorld().submit(new Tickable(1) {
				...
				public void execute() {
					this.stop();
					npc.playProjectile(
							Projectile.create(npc.getCentreLocation(), victim, 448, 25, 5, speed, 43, 36, 10, 48));
				}
